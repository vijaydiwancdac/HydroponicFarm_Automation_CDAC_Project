...
/*
SemaphoreHandle_t xDHT22Done;
SemaphoreHandle_t xTDSDone;
SemaphoreHandle_t xUltraDone;*/
...
int main(void)
{
  ...
  HAL_NVIC_EnableIRQ(ADC_IRQn);
    ...
    /*xDHT22Done = xSemaphoreCreateBinary();
    xTDSDone = xSemaphoreCreateBinary();
    xUltraDone = xSemaphoreCreateBinary();*/
    ...
    /*assert_param(xTaskCreate(DHT22Task, "DHT22Task", STACK_SIZE, NULL, 3,NULL) == pdPASS);*/
    assert_param(xTaskCreate(CommTask, "CommTask", STACK_SIZE, NULL, 3, NULL) == pdPASS);
    assert_param(xTaskCreate(DisplayTask, "DisplayTask", STACK_SIZE, NULL, 3, NULL) == pdPASS);
    ...
    while (1) {
    	...
    	if (status == 0){
    		...
    		if (xSemaphoreTake(xSensorDataMutex, pdMS_TO_TICKS(50)) == pdTRUE) {
                ...
                xSemaphoreGive(xSensorDataMutex);
                //xSemaphoreGive(xDHT22Done); // signal display task
    		}
    		/*
    		else
    		{
                SEGGER_SYSVIEW_PrintfHost("DHT22Task: Failed to acquire mutex");
    		}
*/
    	...
    	/*xSemaphoreGive(xDHT22Done); // signal display task*/
    	/*vTaskDelay(1800/portTICK_PERIOD_MS); // Avoid tight loop*/
    }
}
...
void CommTask(void *argument){
	...
	while(1) {
        ...
        /*xSemaphoreTake(xDHT22Done, 100);
        xSemaphoreTake(xTDSDone, 100);
        xSemaphoreTake(xUltraDone, 100);*/
        /*if(sensorData.dht22Ready == 1 && sensorData.ultrasonicReady==1 && sensorData.tdsReady==1 ){*/
        if (xSemaphoreTake(xSensorDataMutex, pdMS_TO_TICKS(100)) == pdTRUE) {
        	snprintf(uartBuffer, sizeof(uartBuffer),"%.1f,%.1f,%.2f,%lu\r\n",sensorData.dhtData.Temperature,sensorData.dhtData.Humidity,sensorData.tdsValue,sensorData.waterLevel);
        	/*snprintf(uartBuffer, sizeof(uartBuffer),"123.4,50.2,456.78,9\r\n");*/
        	...
        	SEGGER_SYSVIEW_PrintfHost("🌡️ Temp: °C | 💧Humidity:%% | Tds : %% | Distance:m");
...
void LCD_Display(DHT22_Data_t Data, float tds, uint32_t level) {
	...
	LCD_Print(buf, 0, 0);
	/*LCD_String(buf);*/
	vTaskDelay(200/portTICK_PERIOD_MS); // Avoid tight loop
/*
	sprintf(buf,"Hello\r\n");
	LCD_String(buf);
	LCD_Command(0xC0);
	sprintf(buf,"Project task\r\n");
	*/
	sprintf(buf,"N:%.2f%% W:%ldcm",tds,level);
	LCD_Print(buf, 1, 0);
...
uint8_t DHT22_ReadData(DHT22_Data_t *data)  {
    ...
    if (DHT22_CheckResponse() != 0){
    	SEGGER_SYSVIEW_PrintfHost("response failes  ");
    	return 1;
    }
